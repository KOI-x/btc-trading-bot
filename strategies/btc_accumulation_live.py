...
class BTCAccumulationStrategy:
    def __init__(self, exchange: str, symbol: str, initial_usd: float, params: Optional[Dict] = None):
        """
    ...
    def fetch_historical_data(self, timeframe: str = '1d', limit: int = 200) -> pd.DataFrame:
        """Obtiene datos históricos del exchange."""
        try:
            logger.info(f"Obteniendo {limit} velas de {self.symbol} en timeframe {timeframe}")
    ...
    def should_buy(self, df: pd.DataFrame) -> bool:
        ...
        bollinger_condition = last_row['close'] < (last_row['lower_band'] * (1 - self.params['bollinger_oversold']))
    ...
    def place_buy_order(self, amount: float) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"[SIMULACIÓN] Orden de compra: {amount} {symbol} a ~${price:.2f}")
            
            # Actualizar saldo simulado
            cost = amount * price
            self.simulated_balance['USDT'] -= cost
            self.simulated_balance['BTC'] += amount
            logger.info(f"[SIMULACIÓN] Saldo actualizado - USD: ${self.simulated_balance['USDT']:.2f}, "
...
def main():
    ...
    parser.add_argument('--bollinger-oversold', type=float, default=0.08,
