...
def evaluar_estrategia(df: pd.DataFrame, params: Dict[str, Any] = None) -> str:
    ...
    try:
        ...
        # Calcular EMAs
        df['EMA_FAST'] = df["Precio USD"].ewm(span=params['ema_fast'], adjust=False).mean()
        df['EMA_MED'] = df["Precio USD"].ewm(span=params['ema_medium'], adjust=False).mean()
        df['EMA_SLOW'] = df["Precio USD"].ewm(span=params['ema_slow'], adjust=False).mean()
        ...
        if 'Volumen' in df.columns:
            ...
            volume_ok = df['Volumen'].iloc[-1] > (df['VOL_MA'].iloc[-1] * params['min_volume_multiplier'])
        else:
        ...
        fast_cross_above_med = (prev['EMA_FAST'] <= prev['EMA_MED']) and (current['EMA_FAST'] > current['EMA_MED'])
        fast_cross_below_med = (prev['EMA_FAST'] >= prev['EMA_MED']) and (current['EMA_FAST'] < current['EMA_MED'])
        
        # Generar señales
        if all(buy_conditions) or (fast_cross_above_med and current['EMA_MED'] > current['EMA_SLOW']):
            logger.info("SEÑAL DE COMPRA - Tendencias alcistas y condiciones favorables")
            return "BUY"
            
        elif all(sell_conditions) or (fast_cross_below_med and current['EMA_MED'] < current['EMA_SLOW']):
