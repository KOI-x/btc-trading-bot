...
def get_halving_phase(current_date: datetime) -> Tuple[str, float]:
    ...
    next_halving = min([d for d in halving_dates if d > current_date], default=last_halving + timedelta(days=1460))
...
def evaluar_estrategia_avanzada(df: pd.DataFrame, capital: float, params: Dict[str, Any] = None) -> dict:
    ...
    try:
        ...
        if params['use_s2f'] and 'block_height' in params and s2f_ratio is not None and s2f_ratio > 50 and s2f_deviation > 1.0:
            signal = 'SELL'
        
        position_size = capital * params['max_leverage'] * risk_multiplier
        
        atr = calculate_atr(df['Precio Max'], df['Precio Min'], df['Precio USD']).iloc[-1]
