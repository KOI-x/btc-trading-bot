...
class BTCAccumulationBacktest:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Dist_Soporte'] = (df['Precio USD'] - df['Soporte_Dinamico']) / df['Soporte_Dinamico']
        df['Dist_Resistencia'] = (df['Resistencia_Dinamica'] - df['Precio USD']) / df['Precio USD']
        
        return df
    
    def calculate_position_size(self, current_price: float, atr: float, rsi: float, 
                               dist_soporte: float) -> float:
        """
        ...
        soporte_factor = min(1.0, dist_soporte * 10)  # Más cerca del soporte = mayor posición
    ...
    def get_buy_conditions(self, row: pd.Series, params: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        cond_bollinger = row['Precio USD'] < row['Lower_Band'] * (1 + bollinger_oversold)
    ...
    def execute_buy(self, date: datetime, price: float, atr: float):
        ...
        logger.info(f"{date} - COMPRA: {btc_bought:.8f} BTC a ${price:,.2f} (${position_size:,.2f} + ${commission:.2f} comisión)")
    ...
    def run(self, df: pd.DataFrame, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        btc_return = ((self.btc_balance / (self.initial_usd / df.iloc[0]['Precio USD'])) - 1) * 100
        
        # Calcular drawdown
        equity_curve = pd.DataFrame(self.equity_curve)
        equity_curve['peak'] = equity_curve['total_equity'].cummax()
        equity_curve['drawdown'] = (equity_curve['total_equity'] - equity_curve['peak']) / equity_curve['peak']
...
def load_historical_data(file_path: str = 'fixtures/price_history/BTC_USD.csv') -> pd.DataFrame:
...
def optimize_parameters(df: pd.DataFrame, initial_usd: float, commission: float) -> Dict[str, Any]:
    """
    ...
    for i, combo in enumerate(combinations, 1):
        ...
        backtest = BTCAccumulationBacktest(initial_usd=initial_usd, commission=commission)
        ...
        try:
            ...
            if i % 10 == 0 or i == total_combinations:
                logger.info(f"Progreso: {i}/{total_combinations} | Mejor BTC: {best_btc:.4f} | Sharpe: {best_sharpe:.2f}")
...
def plot_results(results: Dict[str, Any]):
    ...
    try:
        ...
        # Valor total en USD
        plt.subplot(2, 1, 1)
        plt.plot(equity['date'], equity['total_equity'], label='Valor Total (USD)', color='blue')
        ...
        plt.subplot(2, 1, 2)
        plt.plot(equity['date'], equity['btc_balance'], label='BTC Acumulados', color='orange')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Backtest de Estrategia de Acumulación de BTC')
    parser.add_argument('--initial-usd', type=float, default=10000.0, help='Capital inicial en USD')
    parser.add_argument('--commission', type=float, default=0.001, help='Comisión por operación (0.001 = 0.1%)')
    parser.add_argument('--rsi-oversold', type=int, default=30, help='Nivel de RSI para considerar sobreventa')
    parser.add_argument('--bollinger-oversold', type=float, default=0.05, 
    ...
    try:
        ...
        end_date = pd.to_datetime(args.end_date) if args.end_date else pd.to_datetime('today')
        df = df[(df['Fecha'] >= start_date) & (df['Fecha'] <= end_date)]
        
        if df.empty:
            raise ValueError("No hay datos disponibles para el rango de fechas especificado")
            
        logger.info(f"Datos cargados: {df['Fecha'].min()} a {df['Fecha'].max()} ({len(df)} días)")
        
        if args.optimize:
            # Ejecutar optimización
            best_params, results_df = optimize_parameters(df, args.initial_usd, args.commission)
            
            # Preguntar al usuario si desea ejecutar el backtest con los mejores parámetros
            if input("\n¿Desea ejecutar el backtest con los mejores parámetros? (s/n): ").lower() == 's':
        ...
        print("="*60)
        print(f"Período: {df['Fecha'].iloc[0].strftime('%Y-%m-%d')} a {df['Fecha'].iloc[-1].strftime('%Y-%m-%d')}")
        ...
        print(f"{'Retorno HOLD (USD):':<30} {(hold_value/args.initial_usd-1)*100:,.1f}%")
        ...
        print(f"{'Variación Precio:':<30} {(btc_price/df.iloc[0]['Precio USD']-1)*100:,.1f}%")
        print("="*60 + "\n")
        ...
        if results['trades']:
            ...
            print("-"*60)
            print(f"{'Año':<6} {'Operaciones':>12} {'BTC Acum.':>12} {'Invertido (USD)':>15} {'Comisión (USD)':>15}")
            print("-"*60)
            for year, data in yearly.iterrows():
                print(f"{year:<6} {data['trades']:>12} {data['btc_amount']:>12.8f} {data['usd_amount']:>15,.2f} {data['commission']:>15,.2f}")
