...
class EMARSITrendBacktest:
    def __init__(self, initial_capital: float = 10000.0, leverage: float = 1.0,
                 stop_loss: float = 0.10, take_profit: float = 0.20,
    ...
    def run(self, df: pd.DataFrame, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        for i in range(50, len(df)):  # Empezamos después de tener suficientes datos para los indicadores
    ...
    def _open_position(self, date: pd.Timestamp, price: float, signal: str):
        ...
        logger.info(f"{date.date()} - {'Compra' if signal == 'BUY' else 'Venta'} de {position_size:.6f} BTC a ${price:.2f}")
    ...
    def _close_position(self, date: pd.Timestamp, price: float, reason: str):
        ...
        # Calcular P&L
        pl_pct = (price - self.entry_price) / self.entry_price * (1 if self.position > 0 else -1)
    ...
    def _calculate_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        avg_win = winning_trades['pl_pct'].mean() * 100 if not winning_trades.empty else 0
        avg_loss = losing_trades['pl_pct'].mean() * 100 if not losing_trades.empty else 0
        
        profit_factor = abs(winning_trades['pl_usd'].sum() / losing_trades['pl_usd'].sum()) \
            if not losing_trades.empty and losing_trades['pl_usd'].sum() != 0 else float('inf')
        ...
        sharpe_ratio = np.sqrt(365) * daily_returns.mean() / daily_returns.std() if not daily_returns.empty else 0
        ...
        return {
            ...
            'avg_win': avg_win,
            'avg_loss': avg_loss,
            'max_win': winning_trades['pl_pct'].max() * 100 if not winning_trades.empty else 0,
            'max_loss': losing_trades['pl_pct'].min() * 100 if not losing_trades.empty else 0,
            'final_capital': self.capital,
            'equity_curve': equity_curve
        }

def load_historical_data(coin: str = 'bitcoin', start_date: str = '2010-01-01') -> pd.DataFrame:
    ...
    try:
        ...
        if df.empty:
            raise ValueError(f"No hay datos disponibles después de la fecha de inicio: {start_date}")
            
        logger.info(f"Datos cargados correctamente. Rango de fechas: {df['Fecha'].min()} a {df['Fecha'].max()}")
...
def plot_results(metrics: Dict[str, Any], df: pd.DataFrame, params: Dict[str, Any]):
    ...
    try:
        ...
        plt.plot(equity_curve['date'], equity_curve['equity'], label='Estrategia', linewidth=2, color='blue')
        plt.plot(df['Fecha'], price_norm, label='HOLD', linewidth=1.5, linestyle='--', color='gray')
        ...
        if 'trades' in metrics and metrics['trades']:
            ...
            if not trades.empty:
                ...
                if not buys.empty:
                    plt.scatter(buys['date'], 
                              [equity_curve['equity'][i] for i in buys.index if i < len(equity_curve)],
                              marker='^', color='green', s=100, label='Compra')
                if not sells.empty:
                    plt.scatter(sells['date'],
                              [equity_curve['equity'][i] for i in sells.index if i < len(equity_curve)],
                              marker='v', color='red', s=100, label='Venta')
...
def print_results(metrics: Dict[str, Any], df: pd.DataFrame, params: Dict[str, Any]):
    ...
    try:
        ...
        print(f"{'Capital Final:':<25} {format_currency(metrics['final_capital'])} {format_currency(initial_price * (1 + hold_return/100))}")
        print(f"{'Retorno Total (%):':<25} {metrics['total_return']:>5.1f}% {hold_return:>14.1f}%")
        ...
        if 'trades' in metrics and metrics['trades']:
            ...
            for trade in trades[-5:]:
                if trade['type'] == 'CLOSE':
                    print(f"{trade['date'].strftime('%Y-%m-%d')} {trade['position_type']:<6} "
...
def main():
    ...
    parser.add_argument('--ema-fast', type=int, default=9,
                      help='Período de la EMA rápida')
    parser.add_argument('--ema-medium', type=int, default=21,
                      help='Período de la EMA media')
    parser.add_argument('--ema-slow', type=int, default=50,
                      help='Período de la EMA lenta')
    parser.add_argument('--rsi-period', type=int, default=14,
                      help='Período del RSI')
    parser.add_argument('--rsi-overbought', type=int, default=70,
                      help='Nivel de sobrecompra del RSI')
    parser.add_argument('--rsi-oversold', type=int, default=30,
                      help='Nivel de sobreventa del RSI')
    ...
    try:
        ...
        logger.info(f"Stop Loss: {args.stop_loss*100:.1f}%, Take Profit: {args.take_profit*100:.1f}%")
        ...
        logger.info(f"  RSI: Período={args.rsi_period}, Niveles={args.rsi_oversold}/{args.rsi_overbought}")
        logger.info("="*60)
        
        # Cargar datos históricos
        logger.info(f"Cargando datos históricos para BITCOIN desde {args.start_date}...")
