...
def calculate_funding_cost(position_size: float, funding_rate: float, days: float) -> float:
...
def run_backtest(
    ...
    for i in range(1, len(df)):
        ...
        if position_type != PositionType.NONE:
            ...
            if (pnl_pct <= -current_stop) or (pnl_pct >= current_take):
                ...
                trades.append({
                    ...
                    'pnl': pnl,
                    'pnl_pct': pnl_pct * 100,
                    'position_type': 'LONG' if position_type == PositionType.LONG else 'SHORT',
                    'capital': capital,
                    'reason': 'SL/TP hit' if pnl_pct <= -current_stop else 'Take Profit'
                })
        ...
        if position_type != PositionType.NONE:
            ...
            position_value = position_size * (1 + (current_price - entry_price) / entry_price)
    ...
    if position_type != PositionType.NONE:
        ...
        pnl_pct = (current_price - entry_price) / entry_price
        pnl = position_size * pnl_pct * (1 if position_type == PositionType.LONG else -1)
        capital += pnl
    ...
    cagr = ((equity_curve[-1] / initial_capital) ** (1/years) - 1) * 100 if years > 0 else 0
...
def plot_results(results: Dict[str, Any], save_path: str = None) -> None:
    ...
    try:
        ...
        ax1.plot(dates, norm_equity, label=f'Estrategia Halving+S2F ({strategy_return:.1f}%)', 
                 linewidth=2, color='#1f77b4')
        ax1.plot(dates, norm_hold, label=f'Comprar y Mantener ({hold_return:.1f}%)', 
        ...
        ax1.set_title('Rendimiento: Estrategia Halving+S2F vs Comprar y Mantener', fontsize=14, pad=20)
...
def backtest(
    ...
    print("="*60)
    print(f"Período: {results['dates'][0].strftime('%Y-%m-%d')} a {results['dates'][-1].strftime('%Y-%m-%d')}")
    print(f"Días de trading: {days}")
    print(f"\n{'MÉTRICA':<25} {'ESTRATEGIA':<15} {'HOLDEAR':<15}")
    print("-"*60)
    print(f"{'Capital Inicial:':<25} ${results['initial_capital']:,.2f}")
    print(f"{'Capital Final:':<25} ${results['final_capital']:,.2f} ${initial_price + (initial_price * hold_return/100):,.2f}")
    print(f"{'Retorno Total (%):':<25} {results['total_return_pct']:>5.1f}% {hold_return:>14.1f}%")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Backtest de estrategia de Halving y S2F')
    parser.add_argument('--save', type=str, help='Ruta para guardar el gráfico de resultados')
    parser.add_argument('--coin', type=str, default='bitcoin', help='ID de la criptomoneda')
    parser.add_argument('--start-date', type=str, default='2012-01-01', help='Fecha de inicio (YYYY-MM-DD)')
    parser.add_argument('--leverage', type=float, default=3.0, help='Nivel de apalancamiento')
    parser.add_argument('--stop-loss', type=float, default=0.10, help='Stop loss (ej: 0.10 para 10%)')
    parser.add_argument('--take-profit', type=float, default=0.30, help='Take profit (ej: 0.30 para 30%)')
    parser.add_argument('--funding-rate', type=float, default=0.01, help='Tasa de financiamiento anual')
