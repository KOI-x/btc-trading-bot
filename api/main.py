...
) -> tuple[PortfolioEvalResponse, list[dict]]:
    """Internal helper to evaluate a portfolio and collect per-coin results."""
...
async def eval_portfolio(request: PortfolioEvalRequest) -> PortfolioEvalResponse:
    ...
    summary, _ = await _evaluate_request(request)
    return summary


@app.post("/api/evaluation/export")
async def export_evaluation(request: PortfolioEvalRequest) -> StreamingResponse:
    """Return a CSV file with evaluation details."""

    summary, rows = await _evaluate_request(request)
    df = pd.DataFrame(rows)
    df["estrategia_vs_hold"] = summary.estrategia_vs_hold
    df["comentario"] = summary.comentario
    csv_bytes = df.to_csv(index=False).encode()
    headers = {"Content-Disposition": "attachment; filename=evaluation.csv"}
    return StreamingResponse(
        io.BytesIO(csv_bytes), media_type="text/csv", headers=headers
    )
